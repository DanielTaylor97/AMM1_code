...
subX1 = X1[int(1e4/renorm):]
b = np.mean(subX1)
rLine = b*np.ones(np.size(subX1))
b1 = "{:.4E}".format(b)
#plt.plot(subtimes, rLine, 'k--', linewidth = 0.5, markersize = 2, label = 'Chaotic Orbit $\chi_1$ ave. = %s' %b1)
print 'b = ', b

Yval_linsp = np.linspace(1e-4, 2e-1, int(steps/renorm))
plotVec = np.ones(int(steps/renorm))
plt.plot(times, (6.31e-3)*plotVec, 'k--', linewidth = 0.5, markersize = 2)
plt.plot((5e4)*plotVec, Yval_linsp, 'k--', linewidth = 0.5, markersize = 2)
